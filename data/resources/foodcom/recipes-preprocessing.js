...
const {
  RAW_RECIPES_CSV_PATH,
  RAW_RECIPES_JSON_PATH,
  RECIPES_TO_EXTRACT,
} = require('./constants');
...
async function main() {
  ...
  const recipesCount = Math.min(RECIPES_TO_EXTRACT, convertedRecipesCount);
  const recipesToSave = getFilteredRecipes(recipes, recipesCount);
  const stringifiedRecipes = JSON.stringify(recipesToSave);
